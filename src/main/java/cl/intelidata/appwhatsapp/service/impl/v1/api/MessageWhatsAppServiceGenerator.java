...
@Slf4j
public class MessageWhatsAppServiceGenerator {
    ...
    public static <S> S createServiceToken(PropertiesConfig properties, Class<S> serviceClass, DominioWhatsApp dominio) {
        log.info("url token: " + dominio.getApiUrl());
        Retrofit.Builder builder = new Retrofit.Builder().baseUrl(dominio.getApiUrl())
                .addConverterFactory(GsonConverterFactory.create());
        Retrofit retrofit = builder.build();
        httpClient.interceptors().clear();
        httpClient.addInterceptor(chain -> {
            Request original = chain.request();
            Request request = original.newBuilder().header("Authorization", "App 123").addHeader("Content-Type", "application/json").build();
